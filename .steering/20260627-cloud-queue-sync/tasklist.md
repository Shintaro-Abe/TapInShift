...
- [x] Slack App Home がクラウド側で表示されることを確認する。
...
- [x] 出勤、退勤、メモ反映、日別編集、丸め設定がクラウド経由で動作する。
...
- [x] 2026-06-29 に再起動後の常駐同期で出勤・退勤が `reflected` になることを確認した。
- [x] DynamoDB に `time_rounding.mode=30m` が保存され、丸め設定変更がクラウド側へ届くことを確認した。
- [x] メモ反映保存時にDynamoDBが `float` を拒否する問題を確認し、分類confidenceを `Decimal` 保存へ修正してLambdaへデプロイした。
