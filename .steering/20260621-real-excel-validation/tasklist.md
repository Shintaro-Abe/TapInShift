...
- [x] 追加テスト: `tests/test_excel_writer.py`（空欄保持・分丸め・日番号/Excelシリアル/曜日付き文字列検索・対象日なし）、`tests/test_service.py`（カンマ・円付き金額、丸め設定、`needs_confirmation`、writer 失敗）、`tests/test_config.py`（丸め方向読み込み）、`tests/test_storage.py`（`app_settings`）、`tests/test_slack_app.py`（丸め単位 select・失敗文言）、`tests/test_timesheet_tools.py`（`show-db` の `app_settings` 互換表示）。全34件 PASS。
