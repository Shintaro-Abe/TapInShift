...
- [x] メモ分類の検証モードがローカルルールであることを記録する。
  - 金額は `amount` のみに入れ、届出内容と経費内容は分離する。
- [x] 届出内容の検証用メモ例と期待値を決める。
  - `渋谷オフィス` -> `W=渋谷オフィス`
  - `渋谷オフィス 新宿駅-渋谷駅 1200円` -> `W=渋谷オフィス`, `Y=新宿駅-渋谷駅`, `AB=1200`
- [x] 経費内容・金額の検証用メモ例と期待値を決める。
  - `新宿駅-渋谷駅 320円` -> `Y=新宿駅-渋谷駅`, `AB=320`
  - `アレア品川、南平⇔市ヶ谷、1134` -> `W=アレア品川`, `Y=南平⇔市ヶ谷`, `AB=1134`
  - `交通費320円` -> 経路がないため確認待ち
...
- [x] 出勤・退勤では任意メモを送信せず、`W/Y/AB` は `メモ反映` または編集モーダルでのみ更新する。
