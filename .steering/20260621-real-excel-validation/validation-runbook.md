...
| `遅延証明あり 交通費1200円` | `W` 列に `遅延証明あり`、`Y` 列に `交通費`、`AB` 列に `1200` |
| `交通費1200円 遅延証明あり` | `W` 列に `遅延証明あり`、`Y` 列に `交通費`、`AB` 列に `1200` |

OpenAI API key を設定していない場合は、ローカルルールで分類する。
現時点のローカルルールは、届出キーワードと経費キーワードが同じメモに含まれる場合、`W/Y/AB` に分離して反映する。
