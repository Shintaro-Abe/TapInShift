...
6. 複合メモの分離精度を運用判断する。
   - 完了済み。届出内容は拠点・建物・駅などの場所情報、経費内容は金額に対応する内容として扱う。
   - ローカル分類ルールでは `渋谷オフィス 新宿駅-渋谷駅 1200円` を `W=渋谷オフィス`, `Y=新宿駅-渋谷駅`, `AB=1200` に分離する。
   - `アレア品川、南平⇔市ヶ谷、1134` は `W=アレア品川`, `Y=南平⇔市ヶ谷`, `AB=1134` に分離する。
